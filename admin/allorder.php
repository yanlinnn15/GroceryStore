            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">All Order</h3>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                            <li class="breadcrumb-item active"><a href="allorder.php">All Order</a></li>
                        </ol>
                    </div>
                ...
                <!-- ============================================================== -->
				<?php 
                    $sql="SELECT * FROM orderp,payment,timeslot,orderstatus,customer where orderp.orderID=payment.orderID and orderp.timeslot=timeslot.timeSlot and orderstatus.osID=orderp.status and orderp.cusID=customer.cusID";
                    $orderpPage=25;
                    ...
                    if(isset($_GET['sort']) && !empty($_GET['sort'])){
                        switch($_GET['sort']){
                            case '10':
                                $sql.=" and orderDate between CURRENT_TIMESTAMP - INTERVAL '9' DAY and CURRENT_TIMESTAMP ";
                                break;
                            case '20':
                                $sql.=" and orderDate between CURRENT_TIMESTAMP - INTERVAL '19' DAY and CURRENT_TIMESTAMP ";
                                break;
                            case '30':
                                $sql.=" and orderDate between CURRENT_TIMESTAMP - INTERVAL '29' DAY and CURRENT_TIMESTAMP ";
                                break;
                                    ...
                                    <div style="float: left;">
                                        <form action="" name="sort" onchange="submit_form()" id="sort" method="post">
                                            <select name="sort" id="" class="custom-select">
                                                ...
                                                <option value="10" <?php if(isset($_GET['sort'])){ if($_GET['sort']==10){?> selected <?php }} ?>>In 10 days</option>
                                                <option value="20" <?php if(isset($_GET['sort'])){ if($_GET['sort']==20){?> selected <?php }} ?>>In 20 days</option>
                                                <option value="30" <?php if(isset($_GET['sort'])){ if($_GET['sort']==30){?> selected <?php }} ?>>In 30 days</option>
                                            </select>
                                        </form>
                                    </div>
                                </div>
...
<?php
    if(isset($_POST['submitsearch'])){
        ...
        if(!empty(trim($keyw))){
            ...
            header("location:".SITEURL."admin/allorder.php?skeyword=".$keyw);
        }else{
...
<?php
    if(isset($_POST['sort'])){
        ...
        $sort=$_POST['sort'];
        $url=SITEURL."admin/allorder.php?";
